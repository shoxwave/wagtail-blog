...
class TextBlock(blocks.TextBlock):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            **kwargs,
            help_text = "This is from my TextBlock (help test is here)",
            # max_length = 5,
            min_length = 2,
            required=False,
        )
    ...
    def clean(self, value):
        ...
        if "wordpress" in value.lower():
            raise ValidationError("We dont't like WordPress here.")
        return value
...
class FAQListBlock(blocks.ListBlock):
    ...
    def clean(self, value):
        ...
        for index, FAQ_block_obj in enumerate(cleaned_data):
            if 'wordpress' in str(FAQ_block_obj['answer']).lower():
